...
data = pd.read_csv("flux_with_photoz.csv")
index = len(data)
...
for num in range(0, index):
    ...
    Max = max(nuF)
    norm_nuF1 = [i/nuF[midnum] for i in nuF]
    norm_nuF2 = [i/Max for i in nuF]



    # 两种photoz
    z1 = data['redshift1'][num]
    wavelength01 = [i/(1+z1) for i in wavelength]
    z2 = data['redshift2'][num]
    wavelength02 = [i/(1+z2) for i in wavelength]
    ...
    plt.subplot(121)
    plt.loglog()
    plt.scatter(wavelength01, norm_nuF1, color="r", linewidth=1)
    ...
    plt.subplot(122)
    plt.loglog()
    plt.scatter(wavelength02, norm_nuF2, color="r", linewidth=1)
